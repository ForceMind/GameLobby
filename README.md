...
v0.2.5 在屏幕内容层上方约 30% 的区域增加获胜弹幕，最多同时显示四行，按“昵称 · 游戏 · 获得金币”从右向左错峰循环；每条弹幕后提供“我也要玩”入口并直接打开对应游戏。新消息按随机间隔投放：当前行仍在显示时依次占用下一行，四行满后进入队列等待首个空位。该弹幕覆盖大厅、游戏、赛事、活动、商城和个人页。商城将多余的购买保障替换为破产保险箱活动入口，展示昨日净损、随机返还比例、每日刷新时间和统计后 3 小时到账规则；全部为原型规则，不触发真实返还。大厅 PC 全屏将热门游戏与今日赢家榜固定为同排，家族 / 派对模块位于最近在玩之后。个人设置中提供接收他人获胜通知、发送自己的获胜通知、向好友展示最近游戏三个隐私开关，原型状态即时反馈但尚未持久化。语言入口和个人设置统一使用自绘下拉菜单。
