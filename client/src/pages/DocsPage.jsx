        ...
        <article className="docs-content">
          ...
          <section className="docs-section">
            ...
            <Logic title="这份文档怎么读">
              ...
              <p>后台配置与审核状态保存在页面内存中，刷新即重置；玩家语言等偏好可保存在本地存储，刷新不等于清除这些偏好。这是刻意的：它用来对齐产品形态和交互，不承担任何真实资产。凡是文档里写着「服务端」的地方，都是需要真正实现的部分。</p>
            </Logic>
          </section>
          ...
          <section className="docs-section">
            ...
            <Logic title="多语言 · 功能逻辑">
              <p>给开发与评审人员看的独立说明首页默认简体中文，不继承浏览器、宿主或此前玩家页的英文选择；显式链接语言参数仍可覆盖。以下语言选择顺序适用于玩家业务页。</p>
              <p>玩家端注册 24 个语种。玩家看到哪一种，按这个顺序决定：链接里带的语言参数最优先，其次是宿主 App 告诉我们的语言，再次是玩家上次自己选的，然后是浏览器语言，都没有就用简体中文。个人页「设置」中的 ProfileSettings 也提供语言选择器；H5 宿主传入的语言会先校验是否已注册。每种语言都用它自己的文字列出。</p>
              <p>简体中文是默认原文，英文是完整兜底基线；其余注册语种的实际可用文案取决于目录覆盖率，注册不等于已完成翻译。所有给玩家看的文字都放在按语言分开的文案表里，每条文案有稳定标识；中文改动会触发其他译文的复核，而非自动宣称已完成。</p>
          ...
          <section className="docs-section">
            ...
            <Logic>
              ...
              <p>游戏在大厅里的排列顺序和哪几款出现在热门推荐，都是运营在后台拖出来的，不是前端写死的。运营改完排序属于配置变更，要走审核发布；但把一款游戏切成维护中属于紧急操作，保存后立刻生效，因为线上出问题时没有时间等审核。游戏字段、状态、地区和门槛使用同一套校验入口；缺必填项、非法范围或空白名单不能保存。</p>
            </Logic>
            <Roles
              front="渲染目录与筛选、按状态决定按钮是否可点、展示游戏说明、发起启动请求并处理加载与失败、把维护公告展示给玩家。"
              server="返回每款游戏当前是否真的可启动、实时在线人数、地区是否允许，以及启动会话本身。"
              admin="维护游戏目录、名称、分类、角标、封面、简介、排序权重、热门推荐、运行状态与维护公告、老虎机参数，以及地区白名单和进入门槛（等级、余额、性别、家族专属）。"
            />
            ...
            <Logic title="门槛怎么运转">
              ...
              <p>当前静态原型的后台草稿和生效版本只保存在本页内存，刷新重置；玩家页读取同源种子配置，尚无跨页面发布同步。示例中 Golden Pharaoh 需要财富等级 5，演示账号为 3，因此锁定；Fish Hunter 为家族专属，演示账号有家族，可以进入。</p>
            </Logic>
          ...
          <section className="docs-section">
            ...
            <Logic title="草稿与生效版本 · 功能逻辑">
              ...
              <p>三种决定的后果不同。通过意味着用快照覆盖生效版本，同时把旧的生效版本压进回滚栈；通用配置驳回时会重置来源草稿到生效版本，翻译任务则只在没有较新翻译草稿时重置；回滚只允许对当前生效任务执行，旧任务不可回滚。通过时会再次校验快照，数据不合法则拒绝发布并留失败记录。灰度、版本审核同步与生产状态均是原型模拟，不代表真实部署。</p>
            ...
            <Fields caption="配置包括幸运转盘、签到奖励、每日任务、金币礼包、月卡、宝箱报价、多语言文案，以及按环境区分的游戏目录和按记录区分的活动地区。活动奖励仍按类型共享。" rows={[
              ['通过并发布', 'approve', '用快照覆盖生效版本，旧版本进入回滚栈；发布前再校验一次。'],
              ['灰度发布', 'gray', '原型模拟状态；不创建真实灰度环境或流量。'],
              ['驳回', 'reject', '通用配置重置草稿；翻译仅在无较新草稿时重置；必须填写原因。'],
              ['回滚', 'rollback', '仅当前生效任务可执行；恢复上一个生效版本，须填写原因；无历史版本时拒绝并留失败记录。'],
              ['暂停 / 恢复', 'pause / resume', '原型模拟状态，不改真实环境或生效配置。'],
...
GET   /api/v1/winners/today        今日赢家榜与最近中奖，榜单最多 10 条`}</code></div>
            ...
            <Fields rows={[
              ['命名协议', '待评审', '计划使用 snake_case，阅读页原型使用 camelCase；落地前须统一或定义映射，不在文档中擅自定稿。'],
            ...
            <Logic title="后台技术评审 · 待定契约与边界"><p>后台发布任务、审批/驳回/仅当前生效任务回滚、账号角色、翻译导入复核、退款与人工调整复核、版本上传验证的真实接口尚未补齐。人工调整来源为 manual_adjust，内部状态为 processing / completed / failed；仅模拟复核，不改真实余额；版本审核只同步原型来源状态，生产部署仍为模拟。最低审核权限是否属于一期依赖、以及 snake_case 与 camelCase 的映射，均待评审；不据此改动既定分期。</p></Logic>
