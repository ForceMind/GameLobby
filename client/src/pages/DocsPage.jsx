...
const adminModules = [
  ...
  ['games', '游戏管理', '目录排序、热门推荐、游戏配置弹窗（基础信息 / 运行状态 / 大厅展示 / 可用地区 / 进入门槛 / Slots 参数）', '运行状态、维护公告与预下架立即生效，其余走草稿审核'],
      ...
      <div className="docs-shell">
        ...
        <article className="docs-content">
          ...
          <section className="docs-section">
            ...
            <SubHead note="下一个增量的设计约定，当前原型尚未实现；实现顺序见本节末尾的分期。">准入规则与进入门槛</SubHead>
            <Logic title="门槛怎么运转">
              ...
              <p>判断门槛需要知道玩家的等级、余额、性别、家族这些信息，这些一律由宿主 App 通过上下文告知，大厅不自己推断。宿主没有告知的字段按不满足处理，和地区一致：未知不能当作放行，否则门槛形同虚设。因此后台在保存一个用到了宿主尚未接入字段的门槛时，会明确提示这条配置目前会挡住所有人。</p>
              <p>白名单是门槛的例外通道，用于内部测试和 VIP 放行：名单里的玩家 ID 跳过全部等级与家族门槛。但白名单不能跳过地区限制——地区是合规边界，不接受例外。</p>
            </Logic>
            ...
            <Fields caption="现有契约已包含 account.id / name / avatar / level、wallet.coins / gems、locale、country。下列字段为门槛判断新增，取值非法一律丢弃并按未知处理。" rows={[
              ['account.gender', "'male' | 'female'", '玩家性别，用于性别门槛。'],
              ['account.wealthLevel', 'number ≥ 0', '玩家财富等级。'],
              ['account.charmLevel', 'number ≥ 0', '玩家魅力等级。'],
              ['account.familyId', 'string', '玩家所属家族标识；无家族时不传。'],
              ['scene', 'string', '大厅当前被嵌在宿主的哪个场景，用于展示场景过滤；缺失时按「游戏中心」处理。'],
            ]} />
            <SubHead>实现分期</SubHead>
            <States items={[
              ['第一期', '财富等级、魅力等级、账户余额、可玩等级、性别、是否家族专属、运营标签，以及卡片锁定态与启动拦截。', '本次要做的范围'],
