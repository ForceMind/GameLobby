...
桌面浏览器的设备框只用于原型预览：固定比例按窗口可用高度反算宽度，圆角16px，上下12px/左右8px的安全区与系统env取较大值。手机视口直接使用系统env，不叠加模拟边框。游戏渲染在设备容器内，进入时仍发送 `setDisplayMode: full`，关闭后发送原来的half/full；宿主应继续按既有协议分配真实视口，不能把CSS模拟尺寸当作原生尺寸请求。短屏内容单独滚动，返回/关闭控件留在容器安全区内。
