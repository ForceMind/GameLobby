...
locale 解析优先级为 URL 参数、宿主上下文、玩家已保存选择、浏览器语言、默认 `zh-Hans`；个人资料「设置」提供语言切换。玩家端注册 24 个语种；简体中文为原文默认值，英文为完整兜底基线，其他注册语种按实际目录覆盖率提供。缺失翻译回退英文并上报缺失 key。
...
写请求的实际字段命名仍待评审：本计划使用 `request_id` / `idempotency_key` / `server_time`，阅读页原型使用 `requestId` / `idempotencyKey` / `serverTime`；必须在接口落地前统一或明确序列化映射，不能默认其中任一协议。后台发布审核、审批/驳回/回滚、账号角色、翻译导入复核、退款与人工调整复核、版本上传验证等接口尚未补齐，以上清单不是已实现接口声明。资产变更必须原子写入钱包流水，奖励领取必须与幂等键绑定；已发布配置不可覆盖，只能生成新版本并支持回滚。
...
待评审边界：一期是否包含最小身份鉴别、服务端操作/环境权限、审计及最小审核权限依赖，尚未决定；本计划不据此改动既定分期。高级灰度和复杂待办的真实服务端契约也待补齐。
...
## 12. P5 第三轮：待办可执行、审核可见、配置弹窗化与个人页合并（2026-09-04）

用户反馈五条：待办的"处理"界面逻辑不清；发布审核看不到具体配置；游戏编辑应是弹窗且字段要全；活动按类型配置不同；个人页战绩与流水无区别应合并。本节记录改动与验证。**仍是纯前端原型，刷新即重置。**

### 待办事项
每条待办绑定跳转目标（模块 + 对象 ID），主操作「去处理」直接跳到该游戏、发布任务、订单或玩家并自动打开。页面顶部写明三个状态的含义；支持认领（归属到操作人）、填写处理结论后关闭、转交他人。关联发布任务被通过/驳回、游戏从维护中恢复时自动关闭对应事项。

### 发布审核
新增 `snapshotDiff(moduleId, before, after)`（纯函数 + 3 项单测），审核抽屉渲染生效版本与待发布快照的逐字段对比，改动项高亮，新增/移除分别标记，并提供「查看来源配置」跳转。游戏目录改为逐字段比对（原先只比对汇总串，导致地区、简介、Slots 参数的改动显示为"无变化"）；未改动项默认折叠。

### 游戏管理
抽屉改为宽弹窗，5 个分组共 21 个字段：基础信息、运行状态、大厅展示、Slots 玩法参数、最近操作。补齐封面资源、排序权重、维护公告文案、预计上线时间；最小投注/赔付线/波动性标注「前台未接入」并留空，不编造数值。校验：维护中必须填公告、即将上线必须填时间、热度 0–100、排序权重 >0。运行状态与维护公告立即生效，其余字段进草稿走审核。

### 活动管理
抽取 `CheckinLadderEditor` / `WheelPrizeEditor` / `MissionListEditor` 三个共用组件，子页面与活动弹窗共用同一份草稿。活动弹窗按类型切换配置区：转盘 = 8 格奖项与概率，签到 = 7 天奖励梯度，任务 = 任务列表。共通字段含适用人群、奖励预算等；参与人数与状态只读。弹窗内改动到保存才写入草稿，取消真正丢弃。

### 个人页战绩与流水
取证确认两者**逐条精确重复**：流水的 `game_reward +3600` / `game_cost -800` 就是战绩里 Golden Pharaoh 与 Fruit Party 那两条，只是流水丢了游戏名；而战绩永远不含签到、任务、宝箱。合并方案：删除 `data.js: recentRecords`，以流水为唯一数据源；预览流水补 `gameId` 与 3 场缺失的游戏事件（金币/宝石余额链条连续，期末 52,860 / 84 与账户余额一致）；游戏类条目显示游戏名；全量视图新增「类型」筛选。个人页两张卡改为**互补分区**——「奖励与消费」与「最近战绩」，同一条流水不再出现两次，`#records` 锚点保留供大厅入口跳转。

### 顺带修复的缺陷
- 抽屉打开后不随 store 刷新：认领待办后抽屉仍显示旧状态与旧按钮。descriptor 改为每次渲染从最新 store 重建，只读字段直接读数据源而非本地草稿。
- `navigate` 未透传给页面包装组件，导致「查看来源配置」按钮不出现。

### 验证
lint 无错误；`npm test` 70/70（新增 3 项差异单测，1 项种子条数断言改为从数据推导）；build 与 check:dist 通过；控制台无报错。浏览器逐条走查：待办跳转与自动关闭、认领与填结论关闭、审核差异与来源跳转、游戏弹窗校验与双保存路径、三种活动类型的不同编辑器与取消/保存、个人页两张卡分区与中英文渲染。

### 仍未做
移动端尺寸未检查；发布审核的灰度/暂停/恢复与游戏版本发布各标签的动作只做代码追溯，未逐一点击；活动壳状态与配置版本仍各自独立；前台流水枚举仍缺 `manual_adjust`。
