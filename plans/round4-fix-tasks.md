...
## 明确记录、不在本轮处理的既有限制
- [已被后续记录替代] 旧结论称"新用户首周礼"与"七日签到·秋日版"共用 `checkin` 模块配置（包括地区）。当前地区已改为按活动记录维护；该旧地区共享结论不再适用。
- 奖励配置仍按活动类型共享；如需按活动实例分别配置奖励，仍需把 `moduleKeys`/`getSlice` 从"按类型"扩展为"按活动实例"，属于更大的架构调整，未包含在本轮范围内。
- 本次文档修复记录为 v0.3.7 修订已部署至 Pages 分支预览，不表示历史 round4 或真实生产服务验收已通过；本轮验证见 `client/QA.md`。
